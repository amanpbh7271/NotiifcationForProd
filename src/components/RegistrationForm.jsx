...
import { TextField, Button, Grid, Typography, MenuItem, Checkbox, ListItemText, Snackbar, Alert } from '@mui/material';
...
const RegistrationForm = () => {
  ...
  useEffect(() => {
    ...
    if (formData.region.length > 0) {
      const regionIds = formData.region.map(region => region.region_id).join(',');
      fetch(`http://inpnqsmrtop01:9090/logtest-0.0.1-SNAPSHOT/api/getAllAccountsForRegions/${regionIds}`)
        .then(response => response.json())
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  SelectProps={{
                    multiple: true,
                    renderValue: (selected) => selected.map(region => region.region_name).join(', '),
                  }}
                  required
                >
                  {regions.map((region) => (
                    <MenuItem key={region.region_id} value={region}>
                      <Checkbox checked={formData.region.some(selectedRegion => selectedRegion.region_id === region.region_id)} />
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  SelectProps={{
                    multiple: true,
                    renderValue: (selected) => selected.map(account => account.account_name).join(', '),
                  }}
                  required
                >
                  {accounts.map((account) => (
                    <MenuItem key={account.account_id} value={account}>
                      <Checkbox checked={formData.account.some(selectedAccount => selectedAccount.account_id === account.account_id)} />
